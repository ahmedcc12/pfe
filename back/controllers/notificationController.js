...
const getAllNotifications = async (req, res) => {
    ...
    try {
        const notifications = await Notification.paginate({ user: req.user._id }, { page, limit, sort: { createdAt: -1 } });
...
const getNotificationsByUser = async (req, res) => {
    ...
    try {
        ...
        const result = await Notification.paginate({ user: req.params.userid }, options);
...
const getUnreadNotificationsByUser = async (req, res) => {
    try {
        const unread = await Notification.countDocuments({ user: req.params.userid, read: false });
        res.send({ unread });
    } catch (error) {
        res.status(500).send(error);
    }
