...
const GroupValidationRules = () => {
    return [
        check('name').trim().escape().notEmpty().withMessage('Name is required').isLength({ min: 3, max: 10 }).withMessage('Name must be at least 3 characters long and at most 10 characters long'),
        check('botIds').isArray({ min: 1 }).withMessage('At least one bot id is required'),
    ];
}
...
const getGroupBotsValidationRules = () => {
    const validSearchOptions = ['all', 'name']
    return [
