  ...
  const fetchBots = async (value) => {
    try {
      ...
      const response = await axiosPrivate.get(
        `/groups/${auth.user.groupId}/bots?page=${
          currentPage + 1
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const respone = await axiosPrivate.delete(
            `/botinstances/status/${auth.user.userId}/${botId}`,
            { signal: abortController.signal }
          );
          //update bot status
          setBots((prevBots) =>
                    ...
                    {bots.map((bot, index) => (
                      <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : ''}>
                        ...
                        <td className="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                          {bot.status === 'inactive' ? (
                            <button
                              onClick={() => openModal(bot._id)}
                              className="bg-green-500  me-2 mb-2 px-5 py-2.5 hover:bg-green-600 text-white font-bold py-2 px-4 rounded"
                              title="Start"
                            >
                            ...
                            <button
                              ...
                              className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded me-2 mb-2 px-5 py-2.5"
                              title="Stop"
                            >
